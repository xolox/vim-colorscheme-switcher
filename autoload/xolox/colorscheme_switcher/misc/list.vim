...
function! xolox#colorscheme_switcher#misc#list#unique(list) " {{{1
...
function! xolox#colorscheme_switcher#misc#list#binsert(list, value, ...) " {{{1
...
if 0
  " Tests for xolox#colorscheme_switcher#misc#list#binsert().
  let s:list = ['a', 'B', 'e']
  function! s:test(value, expected)
    call xolox#colorscheme_switcher#misc#list#binsert(s:list, a:value, 1)
    if s:list != a:expected
      call xolox#colorscheme_switcher#misc#msg#warn("list.vim: Test failed! Expected %s, got %s",
